...
function adjustLightness(rgb, targetLightness) {
  const hsl = rgbToHsl(rgb);
  hsl.l = Math.max(0, Math.min(100, targetLightness));
  return hslToRgb(hsl);
}
function findAccessibleColor(foreground, background, targetRatio, adjustForeground = true) {
  ...
  while (iterations < maxIterations && maxL - minL > 1) {
    ...
    const ratio = adjustForeground ? getContrastRatio(adjusted, fixedColor) : getContrastRatio(fixedColor, adjusted);
  ...
  const lightRatio = adjustForeground ? getContrastRatio(lightAdjusted, fixedColor) : getContrastRatio(fixedColor, lightAdjusted);
  const darkRatio = adjustForeground ? getContrastRatio(darkAdjusted, fixedColor) : getContrastRatio(fixedColor, darkAdjusted);
...
// src/wcag-contrast.ts
function getContrastRequirements() {
  return {
    "normal-text": { AA: 4.5, AAA: 7 },
    "large-text": { AA: 3, AAA: 4.5 },
    "ui-component": { AA: 3, AAA: 3 }
    // AAA doesn't have stricter requirements
  };
}
function getWCAGGuideline(contentType) {
  switch (contentType) {
    case "normal-text":
    case "large-text":
...
function analyzeColorPair(foreground, background, contentType = "normal-text", level = "AA") {
...
function suggestAccessibleColors(foreground, background, targetRatio, preserveProperty = "both") {
  const foregroundRgb = parseColor(foreground);
  const backgroundRgb = parseColor(background);
  const suggestions = [];
...
async function main() {
  ...
  console.error("AccessLint MCP Server running on stdio");
}
main().catch((error) => {
